...
function diffPenFiles(
  ...
  result.stats.total = result.stats.addCount + result.stats.delCount +
                       result.stats.modCount + result.stats.moveCount
...
function diffNodeProperties(
  ...
  return patches.map(patch => ({
    ...
    oldValue: patch.op === 'replace' || patch.op === 'remove' ?
              getValueAtPath(oldProps, patch.path) : undefined,
    newValue: patch.op === 'replace' || patch.op === 'add' ?
